...
public class Cnab400Service {
    ...
    Step stepCnab(ItemReader<Cnab400> reader, ItemProcessor<Cnab400, Cnab> processor, ItemWriter<Cnab> writer) {
        return new StepBuilder("CNAB_400_STEP", repository)
                .<Cnab400, Cnab>chunk(200, platformTransactionManager)
                .reader(reader)
                .processor(processor)
                .writer(writer)
                .build();
    ...
    @Bean
    @StepScope
    FlatFileItemReader<Cnab400> readerCnab(@Value("#{jobParameters['cnabFile']}") Resource resource) {
        return new FlatFileItemReaderBuilder<Cnab400>()
                ...
                .columns(new Range(1, 1), new Range(2, 6), new Range(7, 7), new Range(8, 12), new Range(13, 19),
                        ...
                        new Range(275, 314), new Range(315, 326), new Range(327, 331), new Range(332, 334), new Range(335, 394), new Range(395))
                .names("identRegistro", "agenciaDebito", "digitoAgencia",
    ...
    @Bean
    ItemProcessor<Cnab400, Cnab> processorCnab() {
        return cnab -> {
            if (cnab.controleParticipante() == null || cnab.controleParticipante().isBlank() || cnab.controleParticipante().isEmpty())
                return null;
    ...
    private LineMapper<Cnab400> lineMapper() {
        ...
        lineTokenizer.setColumns(new Range(1, 1), new Range(2, 6), new Range(7, 7), new Range(8, 12), new Range(13, 19),
                ...
                new Range(335, 394), new Range(395));

        BeanWrapperFieldSetMapper<Cnab400> fieldSetMapper = new BeanWrapperFieldSetMapper<>();
        fieldSetMapper.setTargetType(Cnab400.class);
